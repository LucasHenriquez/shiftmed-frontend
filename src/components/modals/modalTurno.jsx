  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animation-fadeIn">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl p-8 relative overflow-y-auto max-h-[90vh]">
        <button onClick={() => setModalCrearTurno(false)} className="absolute top-6 right-6 text-slate-400 hover:text-red-500 transition"><X className="w-6 h-6" /></button>
        <div className="flex items-center gap-3 mb-6"><div className="w-12 h-12 bg-emerald-100 rounded-full flex items-center justify-center text-shiftmed-green"><Activity className="w-6 h-6" /></div><div><h3 className="text-2xl font-black text-slate-900 leading-tight">{modoEdicion ? 'Editar Oferta de Turno' : 'Publicar Nuevo Turno'}</h3></div></div>
        ...
        <form className="space-y-4" onSubmit={handleCrearTurno}>
          <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200 mb-2">
            ...
            <div>
              ...
              <select required value={nuevoTurno.especialidad} onChange={(e) => setNuevoTurno({...nuevoTurno, especialidad: e.target.value})} className="w-full px-4 py-3 border border-slate-300 rounded-xl focus:outline-none focus:border-shiftmed-green bg-white font-medium shadow-sm">
                <option value="" disabled>Selecciona especialidad...</option><option value="Medicina General">Medicina General</option><option value="Pediatría">Pediatría</option><option value="Traumatología">Traumatología</option><option value="Ginecología">Ginecología</option>
              </select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select required value={nuevoTurno.tipo_servicio} onChange={(e) => setNuevoTurno({...nuevoTurno, tipo_servicio: e.target.value})} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:border-shiftmed-green bg-slate-50 font-medium">
                <option value="" disabled>Ej: Urgencias, Consulta...</option><option value="Consulta Médica">Consulta Médica</option><option value="Urgencias">Urgencias</option><option value="Pabellón Quirúrgico">Pabellón Quirúrgico</option><option value="Telemedicina">Telemedicina</option>
              </select>
            </div>
            <div><label className="block text-sm font-bold text-slate-700 mb-1">Ubicación Específica</label><input type="text" required value={nuevoTurno.ubicacion_especifica} onChange={(e) => setNuevoTurno({...nuevoTurno, ubicacion_especifica: e.target.value})} placeholder="Ej: Piso 3, Box 14" className="w-full border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-shiftmed-green bg-slate-50 font-medium text-slate-700" /></div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select required value={nuevoTurno.horario} onChange={(e) => setNuevoTurno({...nuevoTurno, horario: e.target.value})} className="w-full border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-shiftmed-green bg-slate-50 font-medium">
                <option value="" disabled>Selecciona horario...</option><option value="08:00 - 14:00 (Diurno)">08:00 - 14:00 (Diurno)</option><option value="14:00 - 20:00 (Tarde)">14:00 - 20:00 (Tarde)</option><option value="20:00 - 08:00 (Noche)">20:00 - 08:00 (Noche)</option>
              </select>
            </div>
          </div>
          ...
          <div><label className="block text-sm font-bold text-slate-700 mb-1">Descripción / Instrucciones Adicionales</label><textarea required rows="2" value={nuevoTurno.descripcion} onChange={(e) => setNuevoTurno({...nuevoTurno, descripcion: e.target.value})} placeholder="Ej: Se requiere puntualidad..." className="w-full border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-shiftmed-green bg-slate-50 font-medium text-slate-700 resize-none"></textarea></div>
